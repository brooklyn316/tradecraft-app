...
function ShareCardModal({
  ...
  const { me, standings, competition } = data;
  const won = me.rank === 1;
  ...
  const beatenBots = standings.filter(s => s.is_bot && me.total_value > s.total_value);
  const lostBots   = standings.filter(s => s.is_bot && me.total_value <= s.total_value);

  // Build the share headline
  const shareHeadline = (() => {
    if (beatenBots.length === 0 && lostBots.length > 0) {
      const bot = BOT_META[lostBots[0].bot_strategy ?? ""];
      const diff = Math.abs(me.return_pct - lostBots[0].return_pct).toFixed(1);
      return `${bot?.name ?? "the bot"} beat me by ${diff}%`;
    }
    if (beatenBots.length === 1) {
      const bot = BOT_META[beatenBots[0].bot_strategy ?? ""];
      const diff = Math.abs(me.return_pct - beatenBots[0].return_pct).toFixed(1);
      return `I beat ${bot?.name ?? "the bot"} by ${diff}%`;
    }
    if (beatenBots.length > 1) {
      return `I beat all ${beatenBots.length} bots`;
    }
    return `I finished ${me.return_pct >= 0 ? "+" : ""}${me.return_pct.toFixed(2)}%`;
  })();
  ...
  const drawCard = useCallback(() => {
    ...
    grd.addColorStop(0, heroColor === "#4ade80"
    ...
    ctx.fillText(medal, W / 2, 76);

    // Return %
    const returnStr = `${me.return_pct >= 0 ? "+" : ""}${me.return_pct.toFixed(2)}%`;
    ctx.font = "bold 52px system-ui, monospace";
    ctx.fillStyle = heroColor;
    ctx.textAlign = "center";
    ctx.fillText(returnStr, W / 2, 148);

    // Headline
    ctx.font = "600 18px system-ui";
    ctx.fillStyle = "rgba(232,234,240,0.85)";
    ctx.textAlign = "center";
    ctx.fillText(shareHeadline, W / 2, 186);

    // Competition name
    ctx.font = "13px system-ui";
    ctx.fillStyle = "rgba(232,234,240,0.4)";
    ctx.textAlign = "center";
    ctx.fillText(competition.name, W / 2, 214);

    // Username tag (bottom left)
    ctx.font = "bold 13px system-ui";
    ctx.fillStyle = "rgba(232,234,240,0.55)";
    ctx.textAlign = "left";
    ctx.fillText(`@${me.username}`, 22, H - 20);

    // Tradecraft brand (bottom right)
    ctx.font = "bold 13px system-ui";
    ctx.fillStyle = "#7dd3b0";
    ctx.textAlign = "right";
    ctx.fillText("TRADECRAFT", W - 22, H - 20);
    ...
    setRendered(true);
  }, [me, competition, shareHeadline, heroColor, medal]);
  ...
  const handleDownload = () => {
    ...
    link.download = `tradecraft-result-${competition.name.replace(/\s+/g, "-").toLowerCase()}.png`;
    ...
    >
      <div style={{
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div style={{ fontSize:13, fontWeight:700, color:"rgba(232,234,240,0.7)", textTransform:"uppercase", letterSpacing:"0.08em" }}>
            Share Your Result
          </div>
          <button onClick={onClose}
            style={{ fontSize:18, background:"none", border:"none", cursor:"pointer",
              color:"rgba(232,234,240,0.4)", lineHeight:1, padding:4 }}>
            ×
          </button>
        </div>

        {/* Canvas preview */}
        <div style={{ borderRadius:10, overflow:"hidden", border:"1px solid rgba(255,255,255,0.08)", lineHeight:0 }}>
        ...
        <div style={{ display:"flex", gap:10 }}>
          <button onClick={handleCopy}
            style={{
              ...
              background: copied ? "rgba(74,222,128,0.15)" : "rgba(125,211,176,0.1)",
              border: `1px solid ${copied ? "rgba(74,222,128,0.4)" : "rgba(125,211,176,0.25)"}`,
              color: copied ? "#4ade80" : "#7dd3b0",
              transition:"all 0.2s",
            }}>
        ...
        <div style={{ fontSize:11, color:"rgba(232,234,240,0.3)", textAlign:"center" }}>
          Post it, tweet it, brag about it — or don't.
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#080f1a", color:"rgba(232,234,240,0.9)", fontFamily:"system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth:960, margin:"0 auto", padding:"40px 28px 80px" }}>
        ...
        {/* ── Actions ── */}
        <div className={revealed ? "reveal" : ""}
          style={{ display:"flex", justifyContent:"center", gap:12, animationDelay:"0.65s", flexWrap:"wrap" }}>
          <button onClick={() => router.push("/dashboard")}
          ...
          <button
            ...
            style={{ padding:"12px 28px", borderRadius:11, fontSize:14, fontWeight:700, cursor:"pointer",
              background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.15)", color:"rgba(232,234,240,0.85)" }}>
            🔗 Share Result
          </button>
          <button onClick={() => router.push("/stats")}
            style={{ padding:"12px 28px", borderRadius:11, fontSize:14, fontWeight:700, cursor:"pointer",
