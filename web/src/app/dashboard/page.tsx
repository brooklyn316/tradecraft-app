...
type RightTab   = "watchlist" | "limits" | "alerts" | "ai" | "movers" | null;
    ...
    <div style={{ minHeight:"100vh", background:"#060a14", color:"#e8eaf0", display:"flex", flexDirection:"column", overflow:"hidden", height:"100vh" }}>
      ...
      <div style={{ display:"flex", flex:1, overflow:"hidden" }}>
        ...
        <aside style={{
          ...
          {/* Tool panel (shown when a right tab is active) — position:relative for StockPredict overlay */}
          {rightTab && (
            <div style={{ flex:1, overflowY:"auto", borderBottom:"1px solid rgba(255,255,255,0.06)", position:"relative" }}>

              {/* StockPredict overlay — shown when Predict button is clicked from Movers */}
              {predictStock && (
                <StockPredict
              ...
              {rightTab === "movers" && (() => {
                const sorted = [...stocks].filter(s => s.change_percent != null);
                const gainers = [...sorted].sort((a,b) => (b.change_percent??0) - (a.change_percent??0)).slice(0,7);
                const losers  = [...sorted].sort((a,b) => (a.change_percent??0) - (b.change_percent??0)).slice(0,7);
                const Row = ({ s }: { s: typeof stocks[0] }) => {
                  const up = (s.change_percent ?? 0) >= 0;
                  return (
                    <div style={{ display:"flex", alignItems:"center", gap:6, padding:"7px 10px", borderBottom:"1px solid rgba(255,255,255,0.04)" }}>
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ fontSize:12, fontWeight:800, color:"rgba(232,234,240,0.85)" }}>{s.symbol}</div>
                        <div style={{ fontSize:10, color:"rgba(232,234,240,0.4)", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{s.company_name ?? ""}</div>
                      </div>
                      <div style={{ textAlign:"right", flexShrink:0 }}>
                        <div style={{ fontSize:11, fontFamily:"monospace", fontWeight:700, color:"rgba(232,234,240,0.7)" }}>${s.price.toFixed(2)}</div>
                        <div style={{ fontSize:11, fontWeight:700, color: up ? "#4ade80" : "#f87171" }}>
                          {up?"+":""}{(s.change_percent??0).toFixed(2)}%
                        </div>
                      </div>
                      <div style={{ display:"flex", flexDirection:"column", gap:4, flexShrink:0 }}>
                        <button onClick={() => { setSelectedStock(s); setCenterTab("chart"); }}
                          style={{ padding:"3px 7px", borderRadius:6, fontSize:10, fontWeight:600, cursor:"pointer",
                            background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", color:"rgba(232,234,240,0.6)" }}>
                          Chart
                        </button>
                        <button onClick={() => setPredictStock({ symbol: s.symbol, price: s.price })}
                          style={{ padding:"3px 7px", borderRadius:6, fontSize:10, fontWeight:700, cursor:"pointer",
                            background:"rgba(125,211,176,0.08)", border:"1px solid rgba(125,211,176,0.25)", color:"#7dd3b0" }}>
                          Predict
                        </button>
                      </div>
                    </div>
                ...
                return (
                  <div>
                    <div style={{ padding:"8px 10px 4px", fontSize:9, fontWeight:700, color:"#4ade80", textTransform:"uppercase", letterSpacing:"0.1em" }}>▲ Top Gainers</div>
                    {gainers.map(s => <Row key={s.symbol} s={s} />)}
                    <div style={{ padding:"10px 10px 4px", fontSize:9, fontWeight:700, color:"#f87171", textTransform:"uppercase", letterSpacing:"0.1em", borderTop:"1px solid rgba(255,255,255,0.05)", marginTop:4 }}>▼ Top Losers</div>
